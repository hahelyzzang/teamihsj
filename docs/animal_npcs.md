...
## 업데이트: 먹이 AND 조건 / 자연 복원

- **먹이는 OR이 아니라 AND**: 동물은 허용된 먹이를 **모두** 받아야 완료돼요.
  각 먹이마다 `무리 수 × 마리당`개가 필요합니다(예: 돼지 5마리 × 2 = 당근·감자·비트
  각 10개씩, 총 30개). 진행 상황은 먹이별로 표시(`Carrot 3/10, Potato 0/10 ...`).
  조달 어려운 먹이는 정리: 말=사과+밀, 아홀로틀/거북/고양이=대구+연어.
- **자연 복원(`NatureRestoration`)**: 무리를 풀어줄 때마다 그 주변 풀밭에서
  짧은 풀과 들꽃(민들레 포함)이 자동으로 자라납니다(바닐라 뼛가루 성장 트리거).
  민들레는 평원 들판에서도 채집할 수 있고, 이 복원으로 공급이 더 늘어납니다.
